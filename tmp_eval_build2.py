...
with open(test_csv, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if expected_doc and expected_doc != 'NONE':
            sku_match = re.search(r'([A-Z]{2,}[-_][A-Z0-9]+[-_][A-Z0-9-]+)', expected_doc)
            if sku_match:
                sku = sku_match.group(1).rstrip('-').rstrip('_')
                if len(sku) >= 5:
                    relevant_skus.append(sku)

        if question_type == 'exact_sku':
            patterns = [
                r'([A-Z]{2,}[-_][A-Z0-9-]+[-_][A-Z0-9-]+)',
                r'(B0[A-Z0-9]{6,10})',
            ]
            for pattern in patterns:
                ...
                for m in matches:
                    m = m.rstrip('-').rstrip('_')
                    if m not in relevant_skus and len(m) >= 5 and 'product' not in m.lower() and 'manuals' not in m.lower():
                        relevant_skus.append(m)

        relevant_skus = list(dict.fromkeys(relevant_skus))[:3]

        if relevant_skus:
            flagged = question_type != 'exact_sku'
            queries.append({
                "query": question,
